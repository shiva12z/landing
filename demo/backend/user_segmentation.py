...
class UserSegmenter:
    ...
    def generate_segment_insights(self) -> Dict:
        ...
        revenue_by_segment = self.user_segments.groupby('customer_segment')['total_revenue'].agg(['sum', 'mean', 'count'])
        
        # Find top revenue segment safely
        if len(revenue_by_segment) > 0:
            revenue_sums = revenue_by_segment['sum']
            # Convert to pandas Series to ensure we have index access
            revenue_series = pd.Series(revenue_sums)
            top_revenue_segment = revenue_series.idxmax()
        else:
            top_revenue_segment = 'Unknown'
